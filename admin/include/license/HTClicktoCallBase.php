<?php
if(!class_exists("HTClicktoCallBase")) {
	class HTClicktoCallBase {
    	public $key = "6887A35B54F02A87";
    	private $product_id = "4";
    	private $product_base = "ht-click-to-call";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_ht-click-to-call_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=ht-click-to-call_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
