...
if ( !class_exists( 'ClickCall_Settings_API' ) ):
class ClickCall_Settings_API {
        ...
        foreach ( $this->settings_fields as $section => $field ) {
            foreach ( $field as $option ) {
                ...
                $args = array(
                    ...
                    'textFieldNo'       => isset($option['textFieldNo']) ? $option['textFieldNo'] :4,
                    'type'              => $type,
    ...
    /**
     * Displays multiple text field for a settings field
     *
     * @param array   $args settings field args
     */
    function callback_multitext( $args ) {
        ...
        $html ='';
        $get_value   = $this->get_option( $args['id'], $args['section'], $args['std'] ) ;
        $size        = isset( $args['size'] ) && !is_null( $args['size'] ) ? $args['size'] : 'regular';
        $placeholder = is_array( $args['placeholder'] ) ? $args['placeholder'] : array('','','','');
        $file_no     = isset( $args['textFieldNo'] ) ? $args['textFieldNo'] : 4;
        $value       = is_array($get_value) ? $get_value : array('','','',''); 

        for ($i=0; $i < $file_no ; $i++) { 
            $html    .= sprintf( '<input type="text" class="%1$s-text" id="%2$s[%3$s]-%3$s" name="%2$s[%3$s][]" value="%4$s" placeholder="%5$s"/>', $size, $args['section'], $args['id'], $value[$i], $placeholder[$i] );
        }
